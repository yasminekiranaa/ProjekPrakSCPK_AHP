...
if halaman == "📂 Dataset":
    ...
    uploaded = st.file_uploader(
        "📤 Upload File CSV",
        type=["csv"],
        help="Upload dataset dengan format .csv"
    )

    if uploaded:
        df = pd.read_csv(uploaded)
        st.session_state["df"] = df
        st.session_state["dataset_nama"] = uploaded.name

        st.success(
            f"✅ Dataset berhasil diupload: **{len(df):,} baris × {len(df.columns)} kolom**"
        )
...
# ══════════════════════════════════════════════════════════════════════
    ...
    if hitung:
        ...
        df_eval = df.dropna(subset=kriteria_cols).head(int(n_alt)).copy()
        ...
        df_result = df_result.sort_values("Nilai Preferensi", ascending=False).reset_index(drop=True)
        ...
        df_chart = df_result.head(top_n_chart).copy()
        ...
        fig, ax = plt.subplots(figsize=(10, max(5, top_n_chart * 0.4)))
        bars = ax.barh(
        ...
        ax.set_title(f"Top {top_n_chart} Alternatif Terbaik — Metode AHP", fontsize=13, fontweight="bold")
        ax.bar_label(bars, fmt="%.4f", padding=4, fontsize=8)
        ax.set_xlim(0, df_chart["Nilai Preferensi"].max() * 1.15)
